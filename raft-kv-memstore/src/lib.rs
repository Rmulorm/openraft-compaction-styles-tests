...
pub async fn start_example_raft_node(node_id: ExampleNodeId, http_addr: String, snapshot_target: u64) -> std::io::Result<()> {
    // Create a configuration for the raft instance.
    let config = Config {
        heartbeat_interval: 500,
        election_timeout_min: 1500,
        election_timeout_max: 3000,
        snapshot_policy: SnapshotPolicy::LogsSinceLast(snapshot_target),
        ..Default::default()
    };
